...
BMP_Image *Convert_24_to_16_BMP_Image_with_Dithering(BMP_Image *image)
{
  ...
  int padding2 =0;
  BMP_Image *image_16 = (BMP_Image *)malloc(sizeof(BMP_Image));
  int *err = (int*)malloc(sizeof(int)*image->header.width*(image->header.bits/8)*image->header.height);   
  ...
  for(i=0;i<image->header.width*3*image->header.height;i++)
    {
      err[i] = 0;     
    }
  ...
  if(image->header.width*(image->header.bits/8) % 4 !=0)
      ...
      for(i=0;i<image->header.height;i++)
	{
	  for(j=0;j<bytes24;j++)
	    {
	      oldpixel  =  image->data[j+bytes24*i+padding2*i]+ (err[j+bytes24*i])/16;
	      ...
	      else if(oldpixel<0)
		{
	      ...
	      if((j+3) < bytes24)
		{
		  err[j+3+bytes24*i] += quant_error * 7;
		
		}
	      if(((j-3) >= 0)&&((i+1)<image->header.height))
		{ 
		  err[j-3+bytes24*(i+1)] += quant_error * 3;
		  		 
		}
	      if((i+1)<image->header.height)
		{		  
		  err[j+bytes24*(i+1)] += quant_error * 5;
		 
		}
	      
	      if(((j+3) < bytes24)&&((i+1)<image->header.height))
		{
		   err[j+3+bytes24*(i+1)] += quant_error * 1;
